...
export default class PostgreSQLParser extends Parser {
  /**
   * parse key
   * @param {String} key
   */
    ...
    if (helper.isObject(group)) {
      ...
      for (let key in group) {
        ...
        if (key.indexOf('.') === -1) {
          ...
          let keys = key.split('.');
          result.push(`"${keys[0]}"."${keys[1]}"${type}`);
        }
      }
      return ` GROUP BY ${result.join(', ')}`;
    }
  ...
  buildUpdateSql(data: any, options: any) {
    const sql = super.buildUpdateSql(data, options);
    if (options.pk) {
      return `${sql} RETURNING ${options.pk}`;
    } else {
      return `${sql} RETURNING *`;
  ...
  buildDeleteSql(options: any) {
    const sql = super.buildDeleteSql(options);
    if (options.pk) {
      return `${sql} RETURNING ${options.pk}`;
    } else {
      return `${sql} RETURNING *`;
