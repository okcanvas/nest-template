...
function getWeeklyRang(timeStamp: Date) {
  var today = moment(timeStamp);
  var startDate = today.startOf('week').isoWeekday(7);
...
export const work = async (_this: any) => {
  _this.logger.log(`==================================================`);
  _this.logger.log(`CRON FOR dailyPurposeNone`);
  _this.logger.log(`==================================================`);

  //  PUSH_004
  const messageId = 'PUSH_004';
  const messages = await _this.mysql.query('SELECT * FROM message_auto WHERE is_enabled = 1 AND auto_id = ?', [messageId]);
  if (messages.length == 0) return;

  const title = '';
  const content = messages[0].message || '성공한 사람들은 하루 계획을 짜서 산다구~! 우선 계획부터 세우고 시작해보자 우리 :)';
  const pushTokens = [];
  ...
  try {
    ...
    //  3일
    const select = 'SELECT userId, pushToken FROM users ' +
                   'WHERE accountState = "normal" AND userId NOT IN ( SELECT DISTINCT(userId) FROM dailyPurpose WHERE DATE_ADD(createdAt, INTERVAL 3 DAY) > NOW() ) ';
    const userList = await conn.query(select);

    for (const user of userList) {
      if (user.pushToken && user.pushToken != '') {
        //  9시간
        const type = 'dailyPurposeNone';
